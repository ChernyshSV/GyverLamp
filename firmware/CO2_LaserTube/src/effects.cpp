...
{
  for (unsigned char i = 0; i < modes[0].scale; i++)
  {
    unsigned char x = random(0, WIDTH);
    unsigned char y = random(0, HEIGHT);
...
void drawFrame(int pcnt)
{
  ...
  for (unsigned char x = 0; x < WIDTH; x++)
  {
    ...
    CRGB color = CHSV(
        ...
        255,                                                                                 // S
        (unsigned char)(((100.0 - pcnt) * matrixValue[0][newX] + pcnt * line[newX]) / 100.0) // V
    );
...
void rainbowVertical()
{
  ...
  for (unsigned char j = 0; j < HEIGHT; j++)
  {
    CHSV thisColor = CHSV((unsigned char)(hue + j * modes[2].scale), 255, 255);
    for (unsigned char i = 0; i < WIDTH; i++)
...
void rainbowHorizontal()
{
  ...
  for (unsigned char i = 0; i < WIDTH; i++)
  {
    CHSV thisColor = CHSV((unsigned char)(hue + i * modes[3].scale), 255, 255);
    for (unsigned char j = 0; j < HEIGHT; j++)
...
void matrixRoutine()
{
  ...
  {
    // заполняем случайно верхнюю строку
    unsigned int thisColor = getPixColorXY(x, HEIGHT - 1);
    if (thisColor == 0)
...
void lightersRoutine()
{
  if (loadingFlag)
  {
    loadingFlag = false;
    randomSeed(millis());
    for (unsigned char i = 0; i < LIGHTERS_AM; i++)
    {
  ...
  if (++loopCounter > 20)
    loopCounter = 0;
  for (unsigned char i = 0; i < modes[17].scale; i++)
  {
    if (loopCounter == 0)
