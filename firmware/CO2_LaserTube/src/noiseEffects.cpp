...
#if (WIDTH > HEIGHT)
usigned char noise[WIDTH][WIDTH];
#else
unsigned char noise[HEIGHT][HEIGHT];
#endif

CRGBPalette16 currentPalette(PartyColors_p);
unsigned char colorLoop = 1;
unsigned char ihue = 0;
...
void fillNoiseLED()
{
  ...
  for (int i = 0; i < MAX_DIMENSION; i++)
  {
    ...
    for (int j = 0; j < MAX_DIMENSION; j++)
    {
      ...
      unsigned char data = inoise8(x + ioffset, y + joffset, z);
      ...
      if (dataSmoothing)
      {
        unsigned char olddata = noise[i][j];
        unsigned char newdata = scale8(olddata, dataSmoothing) + scale8(data, 256 - dataSmoothing);
        data = newdata;
      }
  ...
  for (int i = 0; i < WIDTH; i++)
  {
    for (int j = 0; j < HEIGHT; j++)
    {
      unsigned char index = noise[j][i];
      unsigned char bri = noise[i][j];
